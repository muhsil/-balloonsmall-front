    ...
    <header className="glass sticky top-0 z-50 border-b border-white/60" style={{ boxShadow: '0 2px 20px rgba(0,0,0,0.07)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 flex items-center justify-between" style={{ height: '72px' }}>
        ...
        <div className="flex items-center gap-3">
          {/* Cart */}
          <Link href="/checkout" className="relative p-2 rounded-xl hover:bg-violet-50 transition-colors group">
            ...
            {cartCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-gradient-to-br from-violet-500 to-pink-500 text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center shadow-md">
                {cartCount}
              </span>
            )}
          </Link>
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-gray-100 bg-white px-4 py-4 flex flex-col gap-3">
          ...
          <Link href="/checkout" className="text-gray-700 font-medium py-2" onClick={() => setMenuOpen(false)}>
